...
var Wiky = {
  ...
  rules: {
     ...
     nonwikiblocks: [
		...
		{ rex:/\$\$(.*?)\$\$/g, tmplt:function($0,$1){return ":p]"+Wiky.store("$$" + Wiky.apply($1, Wiky.rules.code) + "$$")+"[p:";} }, // MathJax (MVC)
		{ rex:/{equation}(.*?)end{equation}/g, tmplt:function($0,$1){return Wiky.store("{equation}" + Wiky.apply($1, Wiky.rules.code) + "end{equation}");} }, // MathJax (MVC)
//MVC				{ rex:/\$(?:\{([^}]*)\})?(?:\(([^)]*)\))?\$(.*?)\$\$/g, tmplt:function($0,$1,$2,$3){return ":p]"+Wiky.store("$"+($2?(" lang=\"x-"+Wiky.attr($2)+"\""):"")+Wiky.style($1)+"$" + Wiky.apply($3, $2?Wiky.rules.lang[Wiky.attr($2)]:Wiky.rules.code) + "$$")+"[p:";} }, // MathJax (MVC)
     ...
     nonwikiinlines: [
     //{ rex:/([^a-z0-9])@([a-z0-9]{5})/g, tmplt:"$1<a href=\"/m/$2\" target=\"_blank\">$2</a>"},  // blab links MVC
      { rex:/\$(?:\{([^}]*)\})?(?:\(([^)]*)\))?(.*?)\$/g, tmplt:function($0,$1,$2,$3){return Wiky.store("$"+($2?(" lang=\"x-"+Wiky.attr($2)+"\""):"")+Wiky.style($1)+"" + Wiky.apply($3, $2?Wiky.rules.lang[Wiky.attr($2)]:Wiky.rules.code) + "$");} }, // inline MathJax (MVC)
      { rex:/%(?:\{([^}]*)\})?(?:\(([^)]*)\))?([^2].*?)%/g, tmplt:function($0,$1,$2,$3){return Wiky.store("<code"+($2?(" lang=\"x-"+Wiky.attr($2)+"\""):"")+Wiky.style($1)+">" + Wiky.apply($3, $2?Wiky.rules.lang[Wiky.attr($2)]:Wiky.rules.code) + "</code>");} }, // inline code [MVC - modified to exclude %2 for space code in html]
      { rex:/%([^2].*?)%/g, tmplt:function($0,$1){return Wiky.store("<code>" + Wiky.apply($1, Wiky.rules.code) + "</code>");} }  //[MVC - modified to exclude %2 for space code in html]
//      { rex:/%(.*?)%/g, tmplt:function($0,$1){return Wiky.store("<code>" + Wiky.apply($2, Wiky.rules.code) + "</code>");} }
     ...
     wikiinlines: [
       ...
       //MVC { rex:/((mailto\:|javascript\:|(news|file|(ht|f)tp(s?))\:\/\/)[A-Za-z0-9\.:_\/~%\-+&#?!=()@\x80-\xB5\xB7\xFF]+)/g, tmplt:"<a href=\"$1\">$1</a>" }  // simple uri's .. 
     ],
     escapes: [
   ...
   style: function(str) {
      ...
      for (var i in s) {
         ...
         else if (p[0] == "=") style += "text-align: center;";//"display:block;margin:0 auto;";  // MVC
...
if (typeof module!=='undefined') module.exports = Wiky